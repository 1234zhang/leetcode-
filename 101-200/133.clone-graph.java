...
class Solution {
    public Node cloneGraph(Node node) {
        ...
        while(!queue.isEmpty()){
            Node no = queue.poll();
            if(!hash.containsKey(no.val)){
                clone = new Node(no.val, new ArrayList<Node>());
                hash.put(clone.val, clone);
            }else{
                clone = hash.get(no.val);
            }
            if(hash.size() == 1) {
                head = clone;
            }   
            for(Node nei : no.neighbors){
                if(!hash.containsKey(nei.val)){
                    Node cloneNei = new Node(nei.val, new ArrayList<Node>());
                    queue.offer(nei);
                    hash.put(cloneNei.val, cloneNei);
                    clone.neighbors.add(cloneNei);
                }
                else{
                    clone.neighbors.add(hash.get(nei.val));
                }
            }
