...
class Solution {
    public int calculateMinimumHP(int[][] dungeon) {
        ...
        int clomn = dungeon[0].length;
        int[] dp = new int[clomn + 1];
        for(int i = 0; i < clomn + 1; i++){
            dp[i] = Integer.MAX_VALUE;
        }   
        dp[clomn - 1] = Math.max(0, -dungeon[row - 1][clomn - 1]);
        for(int i = row - 1; i >= 0; i--){
            for(int j = clomn - 1; j >= 0; j--){
                if(i == row - 1 && j == clomn - 1){
                    continue;
                }
                int min = Math.min(dp[j], dp[j + 1]) - dungeon[i][j];
                dp[j] = Math.max(0, min);
            }
        }
        return dp[0] + 1;
